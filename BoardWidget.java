...
class BoardWidget extends Pad {
    ...
    private void drawPiece(Graphics2D g, Square s) {
        if (this._board.get(s).side() == BLACK) {
            g.setColor(BLACK_COLOR);
            g.drawRect(cx(s), cy(s), SQUARE_SIDE, SQUARE_SIDE);
        } else if (this._board.get(s) == WHITE) {
            g.setColor(WHITE_COLOR);
            g.drawRect(cx(s), cy(s), SQUARE_SIDE, SQUARE_SIDE);
        } else if (this._board.get(s) == KING) {
            g.setColor(WHITE_COLOR);
            g.drawRect(cx(s), cy(s), SQUARE_SIDE, SQUARE_SIDE);
            g.setFont(KING_FONT);
            g.drawString("K", cx(s), cy(s));
        }
    }
